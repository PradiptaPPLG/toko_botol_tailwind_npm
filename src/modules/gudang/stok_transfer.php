...
// Proses stok transfer
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['batch_stok_transfer'])) {
    $cart_items = json_decode($_POST['cart_data'], true);
    $cabang_asal = intval($_POST['cabang_asal']);
    $cabang_tujuan = intval($_POST['cabang_tujuan']);
    $keterangan = escape_string($_POST['keterangan']);

    if (!empty($cart_items)) {
        $batch_id = 'ST-' . date('Ymd') . '-' . rand(1000, 9999);
        $success_count = 0;
        $errors = [];
        ...
        foreach ($cart_items as $item) {
            ...
            if ($stok_asal >= $jumlah) {
                ...
                $sql = "INSERT INTO stok_keluar (produk_id, jumlah, kondisi, cabang_asal, cabang_tujuan, keterangan, batch_id)
                        VALUES ($produk_id, $jumlah, 'transfer', $cabang_asal, $cabang_tujuan, '$keterangan', '$batch_id')";
                execute($sql);
                $success_count++;
            } else {
                $errors[] = $item['nama'] . " - Stok tidak cukup di cabang asal (tersedia: $stok_asal btl)";
            }
        }

        if ($success_count > 0) {
            $success = "✅ Transfer berhasil! $success_count produk ditransfer. Batch: $batch_id";
        }
        if (!empty($errors)) {
...
?>
    ...
    <div class="p-4 lg:p-6">
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-4">
            <h1 class="judul text-2xl lg:text-3xl font-bold text-gray-800 flex items-center">
                <span class="mr-3">🔄</span> STOK TRANSFER
            </h1>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div class="lg:col-span-2">
                <div class="bg-white rounded-lg shadow p-4">
                    <div class="mb-4">
                        ...
                        <div class="mb-3">
                            <label class="block text-sm font-medium mb-2 text-gray-700">📍 Dari Cabang:</label>
                            <label for="select-cabang-asal"></label><select id="select-cabang-asal" onchange="loadProdukByCabang()" class="w-full border-2 border-blue-300 rounded-lg p-2 text-sm font-semibold bg-blue-50">
            ...
            <div class="lg:col-span-1">
                <div class="cart-sidebar bg-white rounded-lg shadow p-4">
                    <h2 class="text-xl font-bold mb-4 text-blue-700">🔄 Transfer</h2>

                    <!-- Cabang Tujuan -->
                    <div class="mb-4">
                        <label class="block text-sm font-medium mb-2 text-gray-700">📍 Ke Cabang:</label>
                        <label for="select-cabang-tujuan"></label><select id="select-cabang-tujuan" class="w-full border-2 border-green-300 rounded-lg p-2 text-sm font-semibold bg-green-50">
                            <?php foreach ($cabang as $c): ?>
                                <option value="<?= $c['id'] ?>"><?= $c['nama_cabang'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="border-t pt-4">
                        ...
                        <form method="POST" id="form-transfer">
                            <input type="hidden" name="batch_stok_transfer" value="1">
                            ...
                            <div class="mb-3">
                                ...
                                <label>
                                    <textarea name="keterangan" rows="2" class="w-full border rounded-lg p-2 text-sm" placeholder="Transfer antar cabang..."></textarea>
                                </label>
                            </div>
    ...
    <script>
        ...
        document.getElementById('form-transfer').addEventListener('submit', function(e) {
            ...
            const cabangAsal = document.getElementById('select-cabang-asal').value;
            const cabangTujuan = document.getElementById('select-cabang-tujuan').value;
            ...
            document.getElementById('form-cabang-asal').value = cabangAsal;
            document.getElementById('form-cabang-tujuan').value = cabangTujuan;
